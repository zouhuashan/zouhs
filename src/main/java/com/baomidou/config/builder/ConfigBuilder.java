...
public class ConfigBuilder {
    ...
    private void handlerPackage(String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.JSP, joinPackage(config.getParent(), config.getJsp()));
        ...
        packageInfo.put("baseys", config.getParent());
        ...
        pathInfo.put(ConstVal.JSP_PATH, joinPath(outputDir, packageInfo.get(ConstVal.JSP)));
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, String tablePrefix) {
        for (TableInfo tableInfo : tableList) {
            ...
            tableInfo.setDegradedFeignClientName(tableInfo.getEntityName() + ConstVal.DEGRADEDFEIGNCLIEN);
            tableInfo.setApiControllerName(tableInfo.getEntityName() + ConstVal.APICONTROLLER);
            tableInfo.setFeignClientName(tableInfo.getEntityName() + ConstVal.FEIGNCLIENT);

            String first = tableInfo.getEntityName().substring(0, 1).toLowerCase();
            String rest = tableInfo.getEntityName().substring(1, tableInfo.getEntityName().length());
            String newStr = new StringBuffer(first).append(rest).append(ConstVal.SERIVCE).toString();
            tableInfo.setServiceNameInController(newStr);
            tableInfo.setEntityNameVO(new StringBuffer(first).append(rest).toString());

            String newStr3 = new StringBuffer(first).append(rest).append(ConstVal.FEIGNCLIENT).toString();
            tableInfo.setServiceNameInfeignClientName(newStr3);
            String newStr4 = new StringBuffer(first).append(rest).append(ConstVal.DEGRADEDFEIGNCLIEN).toString();
            tableInfo.setServiceNameIndegradedFeignClientName(newStr4);
            String newStr5 = new StringBuffer(first).append(rest).append(ConstVal.APICONTROLLER).toString();
            tableInfo.setServiceNameInapiControllerName(newStr5);



            String f1 = tableInfo.getMapperName().substring(0,1).toLowerCase();
            String rest1 = tableInfo.getMapperName().substring(1, tableInfo.getEntityName().length());
            String newStr1 = new StringBuffer(f1).append(rest1).append("Dao").toString();
            tableInfo.setMapperNameInServiceImpl(newStr1);
        }
        return tableList;
    }
